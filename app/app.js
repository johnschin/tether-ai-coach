...
async function handleSignIn() {
  ...
  if (!email || !password) {
    document.getElementById('auth-error').textContent = 'Please enter your email and password.';
    return;
  }
  btn.disabled = true;
...
async function handleSignUp() {
  ...
  if (!email || !password) {
    document.getElementById('auth-error').textContent = 'Email and password are required.';
    return;
  }
...
function setScore(stage, score) {
  ...
  const buttons = document.querySelectorAll(`#scale-${stage} button`);
  ...
  if (confirm('End this session? Tether will save a summary to remember your progress.')) {
    ...
    document.getElementById('messages').innerHTML = `
      <div class="welcome-msg">
        ...
        Your progress has been noted. Come back whenever you need.
      </div>`;
    showScreen('adkar-screen');
  }
}
